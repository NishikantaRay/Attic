...
- **A third blog post**, [the-answer-was-in-a-tab.md](blog/the-answer-was-in-a-tab.md):
  half of what you learn is not in your codebase, and the design calls behind
  the extension — why the companion reuses the CLI writer rather than
  reimplementing it, why editing replaces where stashing appends, and why there
  is no delete. It makes no performance claim; there is no benchmark of
  clipping, so there is no number to quote.
- **"Works with" social cards** (`assets/social/works-with.png` and its square
  form), built by `npm run make:works-with`: Claude Code, Codex CLI and the
  Chromium browsers all writing into one `.attic/`. Third-party names are set
  as wordmarks in Attic's own typeface rather than redrawn — Attic's mark is the
  only logo on the card.
